...
from senzing_grpc import SzDiagnosticGrpc as SzDiagnosticTest
from senzing_grpc import SzEngineGrpc as SzEngineTest
from senzing_grpc import SzError
...
def test_constructor() -> None:
    ...
    actual = SzDiagnosticTest(grpc_channel=grpc_channel)
...
def test_context_managment() -> None:
    ...
    grpc_url = "localhost:8261"
    grpc_channel = grpc.insecure_channel(grpc_url)
    with SzDiagnosticTest(grpc_channel=grpc_channel) as sz_diagnostic:
        actual = sz_diagnostic.get_datastore_info()
...
def szdiagnostic_fixture() -> SzDiagnosticTest:
    """
    ...
    grpc_url = "localhost:8261"
    grpc_channel = grpc.insecure_channel(grpc_url)
    result = SzDiagnosticTest(grpc_channel=grpc_channel)
    return result
